...
ggplot(data = sdr_data, aes(x= goal_1_score, y = goal_2_score, color = goal_6_score)) +
  geom_point()
```
Countries with high SDG 6 (clean water and sanitation) scores tend to have higher SDG 1 (no poverty) scores. Their SDG 2 (zero hunger) score tends to be in the mid to high range. There are very few countries that do not seem to follow this trend. This could mean that with cleaner water and sanitation could be correlated to the amount of poverty present.
...
ggplot(sdr_data, aes(x = goal_1_score, fill=regions_used_for_the_sdr)) + 
  geom_histogram(bins=33, color="black")
```
Interpretation: Countries that fall in the higher end of the Goal 1 spectrum belong to the E. Europe, C. Asia, East/South Asia, OECD, and some MENA countries. Countries that are in the lower spectrum of Goal 1 tend to belong to Sub-Saharan Africa and some MENA countries.
