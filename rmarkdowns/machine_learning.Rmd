...
```{r}
rf_statperf <- randomForest(normalized_score_sdg17_statperf ~ .,
...
```{r}
importance_df <- as.data.frame(rf_statperf$importance)
```
...
```{r}
pdp::partial(rf_statperf, pred.var = "normalized_score_sdg6_sanita", plot = TRUE)
```
...
```{r}
pd <- pdp::partial(rf_statperf, pred.var = c("normalized_score_sdg6_sanita", "normalized_score_sdg17_statperf"))
# Default PDP
plotPartial(pd)
```
