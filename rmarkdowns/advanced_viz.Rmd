...
```{r, warning = FALSE}
goal_3_plot <- ggplot(data = sdr_data, aes(x= goal_3_score, y = goal_4_score, color = regions_used_for_the_sdr)) +
  geom_point() +
  theme_minimal()+
  scale_fill_viridis_d()+
  labs(title = "SDG: Goal 3 vs Goal 4 Scores",
       x = "SDG 3 Score",
       y = "SDG 4 Score",
       fill = "Region")
...
leaflet(sdr_data_world_joined) %>% 
  addTiles()  %>% 
  setView( lat=10, lng=0 , zoom=2) %>%
  addPolygons(stroke = FALSE, fillOpacity = 0.5, smoothFactor = 0.5, color = ~colorQuantile("PuRd", goal_1_score)(goal_1_score), label = mytext)
```
